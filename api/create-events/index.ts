...
const createEvents = async (req, res) => {
  try {
    // Extract events and title from the request body
    const { events, title } = req.body;
    if (!events || !Array.isArray(events) || !title) {
      return res
        .status(400)
    ...
    if (!sub) {
      ...
      return res.status(400).json({ error: 'Failed to extract user identifier' });
    }
    ...
    if (userResult.rowCount === 0) {
      const newUser = await query(`INSERT INTO users (sub) VALUES ($1) RETURNING id`, [`${GOOGLE_OAUTH_PREFIX}:${sub}`]);
    ...
    await query(
      ...
      [JSON.stringify(events), title, userId, newUuidEvent]
    );
