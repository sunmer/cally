...
type ScheduleContextType = {
  ...
  loading: boolean;
  error: string | null;
  suggestSchedule: (query: string) => Promise<boolean>;
...
export const ScheduleProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const suggestSchedule = async (query: string): Promise<boolean> => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const processBuffer = () => {
          // Extract schedule title if not already found
          if (!scheduleTitle) {
          ...
          // Extract the requiresAdditionalContent flag
          ...
          // Update the schedule object if title and flag are found
          ...
          // Regex to find potential events in the current buffer
          ...
          if (eventMatches.length > 0) {
            ...
            // As soon as we find a new event and haven't scrolled yet...
            if (newEvents.length > 0 && !firstEventFound) {
              firstEventFound = true;
              // Scroll down the page (adjust 'top' as needed)
              window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
              // Resolve the promise immediately with true
              resolve(true);
            }
          ...
          // Remove processed event strings from the buffer
          ...
          if (done) {
            ...
            setSchedule(prevSchedule => {
              ...
              return {
                ...
                requiresAdditionalContent:
                  requiresAdditionalContent !== undefined
                    ? requiresAdditionalContent
                    : prevSchedule.requiresAdditionalContent,
                events: allEvents
              };
            });
            break;
